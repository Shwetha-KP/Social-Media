    ...
    <section className="w-full flex flex-col justify-center items-center mt-[50px] mb-[50px]">
      <div className="w-[80%] flex flex-col justify-center gap-y-5 relative">
        <div className="w-full flex justify-start items-center gap-x-5">
          <Image src={profileImage} alt="Profile Image" width={200} height={200} className="sm:w-[200px] sm:h-[200px] xs:w-[100px] rounded-full aspect-square object-cover" />
          <div className="w-[60%] flex flex-col justify-center">
            <p className="text-lg text-gray-500 xs:text-sm">Welcome Back</p>
            <Link href="/userProfile" className="text-2xl font-bold cursor-pointer xs:text-sm">
            Sakshi Agarwal
        ...
        {cards.map((card) => {
          return (
            <Card className="w-full rounded-3xl flex flex-col justify-center" style={{ backgroundColor: card.background }} key={card.id}>
              <CardHeader>
                <CardTitle className="flex items-center gap-4">
                  <Image src={card.profile} alt={card.name} width={50} height={50} className="rounded-full aspect-square object-cover" />
                  <div>
                    <p>{card.name}</p>
                    <p className="text-sm text-gray-500 font-light mt-2">{card.status}</p>
                  </div>
                </CardTitle>
              ...
              <CardContent className="grid justify-items-center">
                <div className="w-full flex justify-center gap-x-5">
                  <Image src={Array.isArray(card.post) ? card.post[0] : card.post} alt={`${card.name}'s post`} width={50} height={50} className="w-[60%] aspect-square object-cover rounded-[30px] xs:rounded-lg" />
                  {card.id === 1 && card.post[1] && (
                    <Image src={card.post[1]} alt={`${card.name}'s additional post`} width={50} height={50} className="w-[38%] aspect-square object-cover rounded-[30px] xs:rounded-lg" />
                  )}
                </div>
              </CardContent>
              ...
              <CardFooter className="flex justify-between items-center">
                <div className="flex items-center ">
                  {card.liked ? (
                    <AiFillHeart color="red" size={24} className="cursor-pointer" onClick={() => handleLike(card.id)} />
                  ) : (
                    <AiOutlineHeart size={24} className="cursor-pointer" onClick={() => handleLike(card.id)} />
                  )}
                  <p className="ml-2">{card.likes}</p>
                </div>
                <Dialog>
                  <DialogTrigger asChild>
                    <Button variant="outline" className="rounded-[30px] bg-[#e3c8f5] ">
                  ...
                  <DialogContent className="sm:max-w-md xs:w-[80%] flex flex-col justify-center">
                    <DialogHeader>
                      ...
                      <ShareSocial
                        ...
                        socialTypes={["facebook", "twitter", "whatsapp", "reddit", "email", "telegram"]}
